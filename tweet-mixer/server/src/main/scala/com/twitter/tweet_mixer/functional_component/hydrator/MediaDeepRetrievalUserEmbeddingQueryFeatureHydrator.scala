...
import com.twitter.tweet_mixer.param.TweetMixerGlobalParams.MediaEvergreenDeepRetrievalModelName
import com.twitter.tweet_mixer.param.TweetMixerGlobalParams.MediaPromotedCreatorDeepRetrievalModelName
...
object MediaDeepRetrievalUserEmbeddingFeature
    extends FeatureWithDefaultOnFailure[PipelineQuery, Option[Seq[Int]]] {
  override def defaultValue: Option[Seq[Int]] = None
}

@Singleton
class MediaEvergreenUserEmbeddingQueryFeatureHydrator @Inject() (
  egsClient: eg.EmbeddingGenerationService.MethodPerEndpoint)
    extends MediaDeepRetrievalUserEmbeddingQueryFeatureHydrator(egsClient) {

  override val identifier: FeatureHydratorIdentifier =
    FeatureHydratorIdentifier("MediaEvergreenDeepRetrievalUserEmbedding")
  override val modelName: FSParam[String] = MediaEvergreenDeepRetrievalModelName
}

@Singleton
class MediaPromotedCreatorUserEmbeddingQueryFeatureHydrator @Inject() (
  egsClient: eg.EmbeddingGenerationService.MethodPerEndpoint)
    extends MediaDeepRetrievalUserEmbeddingQueryFeatureHydrator(egsClient) {

  override val identifier: FeatureHydratorIdentifier =
    FeatureHydratorIdentifier("MediaPromotedCreatorDeepRetrievalUserEmbedding")
  override val modelName: FSParam[String] = MediaPromotedCreatorDeepRetrievalModelName
}

@Singleton
class MediaDeepRetrievalUserEmbeddingQueryFeatureHydrator @Inject() (
  egsClient: eg.EmbeddingGenerationService.MethodPerEndpoint)
  ...
  override val identifier: FeatureHydratorIdentifier =
    FeatureHydratorIdentifier("MediaDeepRetrievalUserEmbedding")

  override val features: Set[Feature[_, _]] = Set(MediaDeepRetrievalUserEmbeddingFeature)

  private val InvalidMediaResponseException = Stitch.exception(
    PipelineFailure(
      InvalidEmbeddingHydrationResponse,
      "Invalid media embedding hydration response"))

  private val FailedMediaResponseException = Stitch.exception(
    PipelineFailure(FailedEmbeddingHydrationResponse, "Failed media embedding hydration response"))
  ...
  override def hydrate(query: PipelineQuery): Stitch[FeatureMap] = {
    getEmbedding(query.getRequiredUserId, query.clientContext, query.params(modelName))
      .map { embedding =>
        new FeatureMapBuilder()
          .add(MediaDeepRetrievalUserEmbeddingFeature, embedding)
          .build()
      }
  }
    ...
    Stitch.callFuture(egsClient.generateEmbeddings(egsQuery)).flatMap {
      case eg.EmbeddingsGenerationResponse
        ...
        userEmbeddingsResult match {
          ...
          case _ => InvalidMediaResponseException
        }
      case _ => FailedMediaResponseException
    }
  }
}
