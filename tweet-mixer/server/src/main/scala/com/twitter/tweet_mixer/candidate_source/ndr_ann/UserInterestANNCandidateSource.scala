...
class UserInterestEmbeddingANNCandidateSource(
  ...
  override def keyTransformer(key: ContentEmbeddingUserANNKey): String =
    f"${sourceIdentifier}_${key.collectionName}_${key.id.toString}_${key.userId.toString}_${key.scoreThreshold.toString}"

  override def enableCache(request: ContentEmbeddingMultipleUserANNQuery): Boolean = request.enableCache

  override def getKeys(
    request: ContentEmbeddingMultipleUserANNQuery
  ): Stitch[Seq[ContentEmbeddingUserANNKey]] =
  ...
  override def getCandidatesFromStore(
    key: ContentEmbeddingUserANNKey
  ...
  override def postProcess(
    request: ContentEmbeddingMultipleUserANNQuery,
    keys: Seq[ContentEmbeddingUserANNKey],
...
@Singleton
class UserInterestEmbeddingANNCandidateSourceFactory @Inject() (
  ...
  def build(identifier: String): UserInterestEmbeddingANNCandidateSource = {
    new UserInterestEmbeddingANNCandidateSource(
      annClient,
      memcacheClient,
