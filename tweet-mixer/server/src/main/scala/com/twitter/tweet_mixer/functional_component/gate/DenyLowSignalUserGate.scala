...
object DenyLowSignalUserGate extends Gate[PipelineQuery] {
  ...
  def evaluate(query: PipelineQuery): Boolean = {
    val followSize = query.features.map(_.getOrElse(SGSFollowedUsersFeature, Seq.empty).size)
    !(SignalUtils.isLowSignalUser(query, followSize) && query.params(EnableLowSignalUserCheck))
  }
}
