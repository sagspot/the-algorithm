...
import com.twitter.tweet_mixer.param.TweetMixerGlobalParams.EvergreenDRUserTweetEnabled
import com.twitter.tweet_mixer.param.TweetMixerGlobalParams.EvergreenDRUserTweetVectorDBCollectionName
import com.twitter.tweet_mixer.param.TweetMixerGlobalParams.EvergreenDRUserTweetMaxCandidates
...
@Singleton
class EvergreenDRUserTweetCandidatePipelineConfig @Inject() (
  ...
  override val identifier: CandidatePipelineIdentifier = CandidatePipelineIdentifier(
    identifierPrefix + CandidatePipelineConstants.EvergreenDRUserTweet)

  override val gates: Seq[Gate[PipelineQuery]] = Seq(
    ParamGate(
      name = "EvergreenDeepRetrievalUserTweetSimilarity",
      param = EvergreenDRUserTweetEnabled
    )
  )
  ...
  override val queryTransformer: CandidatePipelineQueryTransformer[
    ...
    val defaultKey = DRANNKey(
      ...
      collectionName = query.params(EvergreenDRUserTweetVectorDBCollectionName),
      maxCandidates = query.params(EvergreenDRUserTweetMaxCandidates)
    )
  ...
  override def candidateSource: CandidateSource[
    ...
    CandidatePipelineConstants.EvergreenDRUserTweet)
