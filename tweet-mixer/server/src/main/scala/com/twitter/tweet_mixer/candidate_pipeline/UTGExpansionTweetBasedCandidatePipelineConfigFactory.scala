...
import com.twitter.tweet_mixer.candidate_source.UTG.UserTweetGraphConsumerBasedCandidateSource
...
import com.twitter.tweet_mixer.param.TweetMixerGlobalParams.UTGTweetBasedExpansionEnabled
...
@Singleton
class UTGExpansionTweetBasedCandidatePipelineConfigFactory @Inject() (
  userTweetGraphConsumerBasedCandidateSource: UserTweetGraphConsumerBasedCandidateSource,
  ...
  ): UTGExpansionTweetBasedCandidatePipelineConfig[Query] = {
    new UTGExpansionTweetBasedCandidatePipelineConfig(
      identifierPrefix,
      signalsFn = signalsFn,
...
class UTGExpansionTweetBasedCandidatePipelineConfig[Query <: PipelineQuery](
  identifierPrefix: String,
  signalsFn: PipelineQuery => Seq[Long],
  userTweetGraphConsumerBasedCandidateSource: UserTweetGraphConsumerBasedCandidateSource,
  outlierDeepRetrievalEmbeddingQueryFeatureHydratorFactory: OutlierDeepRetrievalEmbeddingQueryFeatureHydratorFactory,
  ...
  override val identifier: CandidatePipelineIdentifier = CandidatePipelineIdentifier(
    identifierPrefix + CandidatePipelineConstants.UTGExpansionTweetBased)

  override val gates: Seq[Gate[PipelineQuery]] = Seq(
    ParamGate(name = "UTGTweetBasedExpansionEnabled", param = UTGTweetBasedExpansionEnabled),
    MaxFollowersGate
  )
  ...
  override val queryTransformer: CandidatePipelineQueryTransformer[
    ...
    identifier = TransformerIdentifier("UTGExpansionTweetBased"),
    query => signalFnSelector(query),
    isExpansionQuery = true,
    minScoreParam = ConsumersBasedMinScoreParam,
