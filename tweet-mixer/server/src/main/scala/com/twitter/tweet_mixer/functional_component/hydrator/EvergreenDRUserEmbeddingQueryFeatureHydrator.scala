...
import com.twitter.tweet_mixer.param.TweetMixerGlobalParams.EvergreenDRUserEmbeddingModelName
import javax.inject.Inject
import javax.inject.Singleton

object EvergreenDRUserEmbeddingFeature
    extends FeatureWithDefaultOnFailure[PipelineQuery, Option[Seq[Int]]] {
  override def defaultValue: Option[Seq[Int]] = None
}

@Singleton
class EvergreenDRUserEmbeddingQueryFeatureHydrator @Inject() (
  egsClient: eg.EmbeddingGenerationService.MethodPerEndpoint)
    extends QueryFeatureHydrator[PipelineQuery] {

  override val identifier: FeatureHydratorIdentifier =
    FeatureHydratorIdentifier("EvergreenDRUserEmbedding")

  override val features: Set[Feature[_, _]] = Set(EvergreenDRUserEmbeddingFeature)
  ...
  override def hydrate(query: PipelineQuery): Stitch[FeatureMap] = {
    getEmbedding(
      ...
      query.clientContext,
      query.params(EvergreenDRUserEmbeddingModelName))
      .map { embedding =>
        new FeatureMapBuilder()
          .add(EvergreenDRUserEmbeddingFeature, embedding)
          .build()
      }
  }
