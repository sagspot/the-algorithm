...
import com.twitter.tweet_mixer.param.TweetMixerGlobalParams.ContentExplorationEmbeddingANNMaxCandidates
import com.twitter.tweet_mixer.param.TweetMixerGlobalParams.ContentExplorationEmbeddingANNScoreThreshold
import com.twitter.tweet_mixer.param.TweetMixerGlobalParams.ContentExplorationEmbeddingSimilarityEnabled
...
import com.twitter.tweet_mixer.param.TweetMixerGlobalParams.ContentExplorationVectorDBCollectionName
...
@Singleton
class ContentExplorationEmbeddingSimilarityCandidatePipelineConfigFactory @Inject() (
  ...
  ): ContentExplorationEmbeddingSimilarityCandidatePipelineConfig[Query] = {
    val candidateSource = contentEmbeddingANNCandidateSourceFactory.build(
      CandidatePipelineConstants.ContentExplorationEmbeddingSimilarity
    )
    new ContentExplorationEmbeddingSimilarityCandidatePipelineConfig(
      candidateSource,
      signalFn,
...
class ContentExplorationEmbeddingSimilarityCandidatePipelineConfig[Query <: PipelineQuery](
  ...
  override val gates: Seq[Gate[PipelineQuery]] = Seq(
    ParamGate(
      name = "ContentExplorationEmbeddingSimilarity",
      param = ContentExplorationEmbeddingSimilarityEnabled
    ),
  ...
  override val queryTransformer: CandidatePipelineQueryTransformer[
    ...
    val collectionName = query.params(ContentExplorationVectorDBCollectionName)
    val maxCandidates = query.params(ContentExplorationEmbeddingANNMaxCandidates)
    val scoreThreshold = query.params(ContentExplorationEmbeddingANNScoreThreshold)
    ...
    val embeddings =
      ...
      else if (query.params(ContentExplorationMultimodalEnabled))
