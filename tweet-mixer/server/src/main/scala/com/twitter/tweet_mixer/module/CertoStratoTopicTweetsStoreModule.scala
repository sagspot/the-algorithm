...
object CertoStratoTopicTweetsStoreModule extends TwitterModule {

  @Provides
  @Singleton
  @Named(ModuleNames.CertoStratoTopicTweetsStoreName)
  def providesCertoStratoTopicTweetsStore(
    ...
    val certoStore = ObservedReadableStore(CertoTopicTopKTweetsStore.prodStore(stratoClient))(
      statsReceiver.scope(ModuleNames.CertoStratoTopicTweetsStoreName)).mapValues {
    ...
    val memCachedStore = ObservedMemcachedReadableStore
      .fromCacheClient(
        ...
        keyToString = { k => s"certo:${KeyHasher.FNV1A_64.hashKey(k.toString.getBytes)}" }
      )
